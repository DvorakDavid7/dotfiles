...
function src () { source "$HOME/.bashrc"; echo "sourced!"; }
...
function cmp() {
    $CC -Wall -pedantic -std=c++17 $1
}

# compile and run cpp program
function cmpr() {
    $CC -Wall -pedantic -std=c++17 $1 && ./a.out
}

# compile and run cpp program with address sanitizer
function cmprs() {
    $CC -Wall -pedantic -std=c++17 -g -fsanitize=address $1 && ./a.out
}
...
function helthCheck() {
    echo "healthfit2022checker" | pbcopy
    ssh healthchecker@10.38.7.16
}
